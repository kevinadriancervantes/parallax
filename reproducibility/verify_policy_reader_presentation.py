...
def verify() -> dict[str, object]:
    ...
    current_html_paths = ("index.html", "experiments/index.html", "experiments/experiment-1/index.html", "experiments/experiment-2/index.html", "experiments/experiment-3/index.html", "evidence/local-surrogate-summary/index.html", "governance/claim-firewall/index.html", "reproduce/index.html", "research/narrative/index.html", "situation-room/index.html")
    current_html = "\n".join((ROOT / path).read_text(encoding="utf-8") for path in current_html_paths)
    current_copy = current_html + "\n" + (ROOT / "situation-room/app.js").read_text(encoding="utf-8")
    current_metadata = "\n".join(line for line in current_html.splitlines() if any(token in line.lower() for token in ("<meta", "<title", "og:", "twitter:")))
    ...
    add("current/failure/track guide", all(term in public for term in ("What it can show now", "Failure signals", "Track over time")), "reader interpretation guide")
